...
import { ArrowLeft, ChevronRight, CreditCard, Lock, ShieldCheck, Smartphone, Wallet } from 'lucide-react';
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {paymentOptions.map((opt) => (
                  ...
                  >
                    <div
                      className={`p-3 rounded-xl w-fit mb-4 transition-all ${
                        paymentMethod === opt.id ? 'bg-brand-red text-white' : 'bg-bg-dark text-text-secondary group-hover:text-brand-red'
                      }`}
                    >
                      <opt.icon className="h-5 w-5" />
                    </div>
                    <p
                      className={`text-[10px] font-black uppercase tracking-widest italic mb-1 ${
                        paymentMethod === opt.id ? 'text-brand-red' : 'text-[var(--text-primary)]'
                      }`}
                    >
                      {opt.name}
                    </p>
