    ...
    <header className="fixed top-0 left-0 w-full z-50 bg-[var(--glass-bg)] backdrop-blur-md border-b border-border-subtle transition-all duration-300">
      <nav className="max-w-[1400px] mx-auto px-6 md:px-10 py-4 md:py-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 md:gap-6">
          <button onClick={() => handleLinkClick('search')} className="p-2 text-text-secondary hover:text-[var(--text-primary)] transition-colors">
          ...
          >
            ...
            <span className="bg-brand-red text-white px-2 py-0.5 rounded-full text-[9px] group-hover:bg-white group-hover:text-black transition-colors">
              {cartCount}
            </span>
          </button>

          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 text-[var(--text-primary)] hover:text-brand-red transition-colors">
