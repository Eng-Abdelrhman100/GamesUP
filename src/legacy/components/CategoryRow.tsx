      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-10">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-4">
            <button onClick={onSeeAll} className="text-[10px] font-black text-text-secondary hover:text-brand-red transition-all uppercase tracking-widest italic">
              Show All Operations
            </button>
        ...
        <div ref={scrollRef} className="flex gap-6 overflow-x-auto scrollbar-hide pb-6 snap-x" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {games.map((game) => (
            <motion.div
              key={game.id}
              whileHover={{ y: -5 }}
              ...
              <div className="relative aspect-[4/5] rounded-3xl overflow-hidden bg-bg-card border border-border-subtle cursor-pointer transition-all hover:border-brand-red/50">
                ...
                <div className="absolute top-4 left-4 flex flex-col gap-2">
                  {game.status !== 'IN STOCK' && (
                    <span className="bg-brand-red text-white text-[8px] font-black tracking-[0.2em] px-3 py-1 rounded-full uppercase italic shadow-lg shadow-brand-red/20">
                      {game.status}
                    </span>
                  )}
                  <span className="bg-black/40 backdrop-blur-md text-white text-[8px] font-black tracking-[0.2em] px-3 py-1 rounded-full uppercase italic border border-white/10">
                    L.E {game.basePrice}
                  </span>
                </div>
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  <h3 className="text-xl font-black text-white tracking-tighter uppercase italic leading-none mb-2">{game.title}</h3>
