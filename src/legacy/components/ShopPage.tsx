...
import { ArrowLeft, Filter, Heart, Search, ShoppingCart, Star, X } from 'lucide-react';
...
export const ShopPage = ({ games, onProductClick, onBack, initialCategory = 'ALL', pageTitle, pageSubtitle, hideCategoriesBar = false, favorites = [], onToggleFavorite }: ShopPageProps) => {
  ...
  const filteredGames = games
    .filter((game) => {
      ...
      const matchesTier =
        tierFilter === 'ALL' || game.accountTypes.some((t) => t.tier === tierFilter && t.isAvailable);
      const matchesMinPrice = minPrice === '' || game.basePrice >= Number.parseInt(minPrice, 10);
      const matchesMaxPrice = maxPrice === '' || game.basePrice <= Number.parseInt(maxPrice, 10);
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`p-3 bg-bg-card border rounded-xl transition-all ${
                  showFilters ? 'border-brand-red text-brand-red' : 'border-border-subtle text-text-secondary hover:text-brand-red'
                }`}
              >
            ...
            {filteredGames.length > 0 ? (
                  ...
                  <div className="relative aspect-[3/4] rounded-[2rem] overflow-hidden bg-bg-card border border-border-subtle cursor-pointer transition-all hover:border-brand-red group-hover:shadow-[0_0_50px_-10px_rgba(235,59,59,0.15)]">
                    ...
                    <div className="absolute top-6 left-6 flex flex-col gap-2">
                      {game.status !== 'IN STOCK' && (
                        <span className="bg-brand-red/90 backdrop-blur-md text-white text-[9px] font-black tracking-[0.2em] px-4 py-2 rounded-xl uppercase italic border border-white/10">
                          {game.status}
                        </span>
                      )}
                      <span className="bg-black/60 backdrop-blur-md text-white text-[9px] font-black tracking-[0.2em] px-4 py-2 rounded-xl uppercase italic border border-white/10">
                        L.E {game.basePrice}
                      </span>
                    </div>
                    ...
                    <div className="absolute bottom-8 left-8 right-8">
                      ...
                      <h3 className="text-2xl font-black text-white tracking-tighter uppercase italic leading-none mb-3">{game.title}</h3>
              ...
              <div className="col-span-full py-32 text-center text-text-secondary">
                ...
                <button
                  ...
                  className="mt-4 text-brand-red font-black uppercase tracking-widest hover:underline italic"
                >
                  Reset Filters
                </button>
              </div>
