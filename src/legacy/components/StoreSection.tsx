...
export const StoreSection = ({ onProductClick, favorites = [], onToggleFavorite }: StoreSectionProps) => {
  return (
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 md:mb-16 gap-6 border-b border-border-subtle pb-8 md:pb-12 transition-colors">
          <div className="text-left">
            ...
            <h2 className="text-5xl md:text-7xl font-black text-[var(--text-primary)] tracking-tighter font-display uppercase italic leading-none transition-colors">
              The Store<span className="text-brand-red">.</span>
            </h2>
          </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {['Action', 'Adventure', 'RPG', 'Shooter', 'Sports'].map((c) => (
                  ...
                  >
                    <span className="text-xs font-bold text-text-secondary group-hover:text-[var(--text-primary)] transition-colors uppercase tracking-widest">
                      {c}
                    </span>
              ...
              {GAMES_DATA.map((game, idx) => (
                ...
                >
                  <div className="relative aspect-[16/10] overflow-hidden bg-gray-900/50">
                    ...
                    <span
                      className={`absolute top-4 right-4 text-[9px] font-black px-2.5 py-1 rounded-sm uppercase tracking-tighter italic transition-colors ${
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    <div className="flex gap-2 mb-2">
                      {game.tags.slice(0, 1).map((tag) => (
                        <span key={tag} className="text-[9px] font-black text-brand-red uppercase tracking-widest italic">
                          {tag} Account
                        </span>
                      ))}
                      <div className="flex text-yellow-500 gap-0.5 text-[8px]">★★★★★</div>
                    </div>
                    <h3 className="font-black text-[var(--text-primary)] text-base tracking-tight uppercase group-hover:text-brand-red transition-colors font-display line-clamp-1 mb-6 italic transition-colors">
                      {game.title}
                    </h3>
                    ...
                    <div className="mt-auto pt-4 flex items-center justify-between border-t border-border-subtle transition-colors">
                      <div className="text-2xl font-black text-[var(--text-primary)] italic tracking-tighter transition-colors">
                        {game.price}{' '}
                        <span className="text-[10px] text-text-secondary font-bold ml-0.5 italic transition-colors">L.E</span>
                      </div>
                      <button className={`btn-action transition-all ${game.status === 'OUT OF STOCK' ? 'opacity-20 cursor-not-allowed' : ''}`}>
