                        ...
                        {order.items.map((item: any, idx: number) => (
                          <div key={idx} className="flex gap-4 p-4 bg-gray-50 rounded-lg">
                            ...
                            <div className="flex-1">
                              ...
                              {/* Show credentials for pending AND completed/delivered orders */}
                              {(['pending', 'delivered', 'completed', 'paid'].includes(order.status)) && (item.allocatedSlots && item.allocatedSlots.length > 0) && (
                                <div className={`mt-3 p-4 rounded-xl border ${order.status === 'pending' ? 'bg-yellow-50 border-yellow-200' : 'bg-green-50 border-green-200'}`}>
                                  <h4 className={`font-bold text-sm mb-3 flex items-center gap-2 ${order.status === 'pending' ? 'text-yellow-800' : 'text-green-800'}`}>
                                    {order.status === 'pending' ? <Clock className="w-4 h-4" /> : <CheckCircle className="w-4 h-4" />}
                                    Digital Account Credentials
                                    {order.status === 'pending' && (
                                      <span className="text-xs font-normal">(Available immediately - Admin approval pending)</span>
                                    )}
                                  </h4>
